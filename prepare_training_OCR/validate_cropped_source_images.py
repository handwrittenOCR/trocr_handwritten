...
def main():
    ...
    # Group samples by source_image for rejected source images only.
    ...
    for src_id, samples in groups_rejected.items():
        ...
        # Build the corrected (cropped) stacked image view.
        corrected_combined = reconstruct_cropped_source_image(samples, crop_factor=0.1)

        # Display side-by-side: left = original, right = corrected.
        fig, axs = plt.subplots(1, 2, figsize=(16, 8))
        axs[0].imshow(original_combined)
        axs[0].set_title(f"Original Combined Image: {src_id}")
        axs[0].axis("off")
        axs[1].imshow(corrected_combined)
        axs[1].set_title(f"Corrected (Cropped) Combined Image: {src_id}")
        axs[1].axis("off")
        plt.tight_layout()
        plt.show()
        plt.pause(0.001)
        ...
        user_input = input("Validate the corrected cropping for this source image? (V)alidate / (R)eject / (E)xit: ").strip().lower()
        plt.close(fig)

        if user_input == "v":
            print(f"Validated cropping for source image: {src_id}")
            # Update each sample from this source; apply the cropping transformation.
            for sample in samples:
                updated_sample = apply_cropping_to_sample(sample, crop_factor=0.1)
    ...
    # Now, prepare the final output dataset.
    # For accepted source images (from initial review), we keep the samples unmodified.
    accepted_samples = []
    for src_id, samples in groups_accepted.items():
        for sample in samples:
            # For consistency, add keys for cropping (here leaving them as the original coordinates)
            sample["cropped_image"] = None  # or you could optionally set to the original image
            sample["cropped_x1"] = sample["x1"]
            sample["cropped_y1"] = sample["y1"]
            sample["cropped_x2"] = sample["x2"]
            sample["cropped_y2"] = sample["y2"]
            accepted_samples.append(sample)

    # Combine the accepted samples with the cropping-validated samples.
    new_dataset_samples = accepted_samples + cropping_validated_samples
    print(f"\nTotal samples in new dataset: {len(new_dataset_samples)}")

    # Create a Hugging Face Dataset from the dictionary.
    new_dataset = Dataset.from_dict(new_dataset_samples)

    # Save the new dataset to disk.
    output_path = "validated_cropped_dataset"
