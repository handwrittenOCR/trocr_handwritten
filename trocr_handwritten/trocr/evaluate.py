...
def evaluate_model(
    ...
    model_settings = OCRModelSettings(
        model_name=model_name, hub_repo=model_name, huggingface_api_key=hf_token
    )
    ...
    ocr_model = OCRModel(
        settings=model_settings, train_settings=None  # Not needed for evaluation
    )
...
def main():
    ...
    parser.add_argument(
        "--hf_token", type=str, help="HuggingFace token for private repositories"
    )
    parser.add_argument(
