...
class pageData:
    ...
    def __init__(self, filepath, logger=None, creator=None):
        ...
        self.creator = "P2PaLA-PRHLT" if creator is None else creator
        self.XMLNS = {
    ...
    def build_mask(self, out_size, element_name, color_dic):
        ...
        for element in element_name:
            for node in self.root.findall("".join([".//", self.base, element])):
                ...
                if e_type is None or e_type not in color_dic:
                    e_color = 175
                    self.logger.warning(
        ...
        if not mask.any():
            self.logger.warning("File {} do not contains regions".format(self.name))
        return mask
    ...
    def build_baseline_mask(self, out_size, color, line_width):
        ...
        if not mask.any():
            self.logger.warning("File {} do not contains baselines".format(self.name))
        return mask
    ...
    def split_image_by_line(self, img, size):
        ...
        for ro_region in sorted(regions):
            self.get_coords(regions[ro_region])
            self.get_id(regions[ro_region])
            lines = {}
